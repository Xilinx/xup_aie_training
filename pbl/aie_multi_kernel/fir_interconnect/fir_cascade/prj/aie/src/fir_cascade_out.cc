...
void fir_8t_16int_cascade_out(input_stream_int16 * __restrict in, output_stream_acc48 * __restrict cascade_out)
{
	...
	for(int i = 0; i<LOOP_CYCLES; i++)
	{
		...
		//Output 8 samples
		//writeincr(cascade_out, acc0.to_vector<acc48>(shift));
		writeincr(cascade_out, acc0);
